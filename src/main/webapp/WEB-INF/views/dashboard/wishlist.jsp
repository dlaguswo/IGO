...
function fn_delete_wishList(r_num) {
	var email = "${sessionScope.guestInfo.email}";
	$.ajax({
		type : "POST",
		async : false,
		url : "${contextPath}/guest/wishListDelete",
		dataType: "text",
		data : {
			email : email,
			r_num : r_num
...
function showWishListForm() {
	$.ajax({
		...
		url: "${contextPath}/guest/wishListForm",
...
function showWishList() {
	$.ajax({
		...
		dataType : "json",
		contentType: "application/json",
		url: "${contextPath}/guest/wishList",
		success:function(result){
			 var output = "";
			 $(result).each(function(){	
				 ...
				 output += "</span>Hotel";
				 output += "</a>";
				 output += "<h3>" + this.r_name + "</h3>";
				 output += "<p class=\"list-address\">" + this.m_addr +  "<i class=\"icofont-google-map\"></i></p>";
				...
				output += "</ul>";
				output += "<div class=\"views\">Views : <span>" + this.hitcount +"</span></div>";
				output += "</div>";
				output += "<div class=\"listing-button\">";	  				
				output += '<a href=\"javascript:void(0)\" class=\"btn v4\" onclick = \"fn_delete_wishList(' + this.r_num + ')\"><i class=\"ion-android-delete\"></i>&nbsp;&nbsp;Delete</a>';
				output += "</div>";
				output += "</div>";			 
			 });
	...
	<!--  왼쪽 사이드 바 -->
		...
		<!--  유저 이미지 -->
			...
			<!-- 사이드 리스트  -->
					...
					<!--  찜목록 -->	
						<li>
							<a class="active" href="<%=cp%>/guest/wishListForm">
						...
						<!--  로그인 시 사용 메뉴 -->
								...
								<!--  mypage 메인 -->
										<div class="account-dropdown__item">
											<a href="<%=cp%>/guest/dashboard">
												<i class="ion-ios-person-outline"></i>Dashboard</a>
										</div>
				...
				<div class="row">
					<div class="col-md-12">
						<div class="recent-activity my-listing">
							<div class="act-title">
								<h5><i class="ion-social-buffer-outline"></i> Wish Listing</h5>
							</div>
